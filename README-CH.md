...
L5  安全 (Security)         → 横向功能 (每个工具的安全级别)
```

> 在 **Mantis-OS** 中，还会增加 L6 (网络/MantisNostr) 和技能 (WORKING/TOOLS/skills/)。
...
│ L1 核心      │                                  │ R1 聊天历史    │
│ L2 身份      │        与代理聊天                 │ R2 项目       │
│ L3 运行时    │        (SSE 流式传输)             │ R3 工作底稿    │
│ L4 工具      │                                  │ R4 工作空间    │
└─────────────┴──────────────────────────────────┴───────────────┘
```

  - **左侧 (代理):** L1 核心 (后端/模型切换), L2 身份 (soul(t) + 检查器), L3 运行时 (健康状态 + 实时 Tick 提要 + 提示词检查器), L4 工具 (注册表)
  - **中间:** 带有 SSE 流式传输（逐个 Token）的聊天界面
  - **右侧 (AAMS):** R1 聊天历史, R2 项目 + 里程碑, R3 工作底稿 (带有关闭切换), R4 工作空间树 + 工作底稿预览
  - **模型切换器:** 在 LM Studio / Ollama 模型之间实时切换
  - **身份检查器:** 在标签页中显示所有 6 个身份文件 (base, agenda, account, social, decentral, hook)
  - **提示词检查器:** 显示最后的 LLM 提示词 (System/User/Response) 用于分析
